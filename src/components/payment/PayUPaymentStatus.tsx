...
import { useGetPayUPaymentStatusQuery } from '@/store/api/payuApi';

interface PayUPaymentStatusProps {
  orderId: number;
}

export const PayUPaymentStatus: React.FC<PayUPaymentStatusProps> = ({
  orderId,
}) => {
  const router = useRouter();

  const { data, isLoading, error } = useGetPayUPaymentStatusQuery(orderId, {
    pollingInterval: 5000,
    skip: !orderId,
  });

  const paymentStatus = data?.data?.status || 'pending';
  ...
  return (
    <div className="rounded-lg border border-gray-200 bg-white p-8 shadow-sm">
      <div className="flex flex-col items-center text-center">
        ...
        <h2 className="mt-4 text-2xl font-bold text-[#1A2A3A]">
          {config.title}
        </h2>
