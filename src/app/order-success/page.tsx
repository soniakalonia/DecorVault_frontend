...
function OrderSuccessContent() {
  ...
  const orderNumber = order
    ...
    : fallbackData.orderNumber;
  const total = order ? Number(order.total) : fallbackData.total;
  const paymentMethod = order
    ? order.payment_method
    : fallbackData.paymentMethod;
  const status = order ? order.status : 'pending';
  const items = order?.items || [];

  // Numeric id we can safely pass to the invoice download button
  const numericOrderId = order?.id || (orderId ? Number(orderId) : null);
        ...
        <div className="bg-white rounded-2xl shadow-2xl overflow-hidden">
          ...
          <div className="p-8">
            ...
            {isError && (
              <div className="bg-yellow-50 border border-yellow-200 rounded-xl p-3 mb-6 text-sm text-yellow-800">
                We couldn't refresh full order details right now, but your
                ...
                <div className="flex items-start gap-3">
                  ...
                  <div>
                    ...
                    <p className="text-sm text-blue-700">
                      Please keep the exact amount ready for delivery. Our
          ...
          <div className="bg-gray-50 px-8 py-4 text-center border-t border-gray-100">
            <p className="text-xs text-gray-400">
              A confirmation email with your invoice has been sent to your
